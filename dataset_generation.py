...
def get_image(args_tuple):
    """
    Download and save a single image.
    args_tuple = (img_url, class_folder)
    """
    img_url, class_folder = args_tuple
    ...
    with lock:
        if class_images.value >= args.images_per_class:
            return
    ...
    try:
        ...
        logging.debug(f"Error saving image {img_url}: {e}")
        return finish('failure')
...
if __name__ == "__main__":
    for class_wnid in classes_to_scrape:
        ...
        # ✅ Use correct folder inside args.data_root
        class_folder_path = os.path.join(imagenet_images_folder, class_name)
